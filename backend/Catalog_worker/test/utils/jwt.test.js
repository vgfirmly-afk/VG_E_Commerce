...
describe('JWT Utils', () => {
  ...
  describe('signJWT', () => {
    it('should sign a JWT token successfully', async () => {
      if (!env.JWT_PRIVATE_KEY || env.JWT_PRIVATE_KEY === 'YOUR_JWT_PRIVATE_KEY_HERE') {
    ...
    it('should include jti in token', async () => {
      ...
      const payloadPart = JSON.parse(atob(parts[1].replace(/-/g, '+').replace(/_/g, '/')));
    ...
    it('should handle custom expiration', async () => {
      ...
      const token = await jwt.signJWT({ sub: 'user123' }, { env, expiresIn: '30m' });
      expect(token).to.be.a('string');
    });
  });
  ...
  describe('verifyJWT', () => {
    ...
    it('should return null for invalid token format', async () => {
      if (!env.JWT_PUBLIC_KEY || env.JWT_PUBLIC_KEY === 'YOUR_JWT_PUBLIC_KEY_HERE') {
        return;
      }
    ...
    it('should return null for expired token', async () => {
      ...
      const token = await jwt.signJWT({ sub: 'user123' }, { env, expiresIn: 1 }); // 1 second expiration
