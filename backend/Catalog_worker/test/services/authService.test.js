...
describe('Auth Service', () => {
  ...
  describe('register', () => {
    ...
    it('should reject registration if email exists', async () => {
      ...
      mockDb.prepare().bind().all.resolves({ results: [existingUser] });
  ...
  describe('login', () => {
    it('should login user successfully', async () => {
      ...
      mockDb.prepare().bind().all.onFirstCall().resolves({ results: [mockUser] }); // User found
    ...
    it('should reject login with wrong password', async () => {
      ...
      const crypto = await import('../../src/utils/crypto.js');
      const salt = crypto.genSalt();
      const hashedPassword = await crypto.hashPassword('correctpassword', salt, env.PEPPER);
      ...
      mockDb.prepare().bind().all.resolves({ results: [mockUser] });
  ...
  describe('rotateRefreshToken', () => {
    it('should rotate refresh token successfully', async () => {
      ...
      mockDb.prepare().bind().all.onFirstCall().resolves({ results: [mockToken] }); // Token found
      mockDb.prepare().bind().all.onCall(1).resolves({ results: [{ id: 'user123', role: 'user' }] }); // User found
    ...
    it('should reject expired refresh token', async () => {
      ...
      mockDb.prepare().bind().all.resolves({ results: [mockToken] });
  ...
  describe('revokeRefreshToken', () => {
    it('should revoke refresh token successfully', async () => {
      ...
      mockDb.prepare().bind().all.onFirstCall().resolves({ results: [mockToken] });
      mockDb.prepare().bind().run.onFirstCall().resolves({ success: true });
  ...
  describe('getUserById', () => {
    it('should return user with decrypted PII', async () => {
      ...
      const crypto = await import('../../src/utils/crypto.js');
      const phoneEncrypted = await crypto.encryptAESGCM('1234567890', env.PII_ENCRYPTION_KEY);
      const addressEncrypted = await crypto.encryptAESGCM('123 Main St', env.PII_ENCRYPTION_KEY);
