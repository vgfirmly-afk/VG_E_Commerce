...
export async function requireAuth(request) {
  ...
  try {
    ...
    if (!auth.startsWith('Bearer ')) return new Response(JSON.stringify({ error: 'Missing token' }), { status: 401, headers: { 'Content-Type': 'application/json' } })
...
export async function requireAuthAdmin(request) {
  ...
  if (!payload || !payload.isAdmin) return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403, headers: { 'Content-Type': 'application/json' } })
  return null
}