...
export async function listProducts(request, env) {
  try {
    ...
    const queryParams = {
      ...
      limit: Math.min(
        parseInt(
          url.searchParams.get("limit") || String(PRODUCTS_PER_PAGE),
          10,
        ),
        MAX_PRODUCTS_PER_PAGE,
      ),
      featured: url.searchParams.get("featured")
        ? parseInt(url.searchParams.get("featured"), 10)
        : undefined,
    ...
    return new Response(
      JSON.stringify({
        products: productsList.map((p) => {
          ...
          if (p.media) {
            const media =
              typeof p.media === "string" ? JSON.parse(p.media) : p.media;
...
export async function getProductImage(request, env) {
  try {
    ...
    const r2Object = await catalogService.getProductImageUrl(
      productId,
      imageId,
      env,
    );
...
export async function getHomePage(request, env) {
  try {
    ...
    const products = await catalogService.getHomePageProducts(
      categories,
      value.limit,
      env,
    );
