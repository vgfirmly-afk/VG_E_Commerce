...
export async function createProduct(request, env) {
  try {
    ...
    const product = await adminService.createProductService(
      value,
      userId,
      env,
      ctx,
...
export async function updateProduct(request, env) {
  try {
    ...
    const product = await adminService.updateProductService(
      productId,
      value,
      userId,
      env,
...
export async function createSku(request, env) {
  try {
    ...
    return new Response(JSON.stringify(sku), {
      status: 201,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err) {
