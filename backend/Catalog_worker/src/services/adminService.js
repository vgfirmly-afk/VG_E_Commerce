...
async function checkPricingWorkerHealth(env) {
  try {
    ...
    if (pricingWorker) {
      ...
      const isHealthy =
        healthResponse.ok && parsedBody && parsedBody.ok === true;

      if (!isHealthy) {
        logError(
          "syncPriceToPricingWorker: Pricing Worker health check failed (service binding)",
          null,
          {
 ...
 */
async function syncPriceToPricingWorker(
  skuId,
  productId,
  skuCode,
  priceData,
  ...
  try {
    ...
    if (pricingWorker) {
      ...
      const pricePayload = {
        ...
        sku_code: skuCode,
        price: priceData.price !== undefined ? priceData.price : 0.0,
        currency: priceData.currency || "USD",
        sale_price:
          priceData.sale_price !== undefined ? priceData.sale_price : null,
        compare_at_price:
          priceData.compare_at_price !== undefined
            ? priceData.compare_at_price
            : null,
        cost_price:
          priceData.cost_price !== undefined ? priceData.cost_price : null,
      ...
      if (!healthCheck.healthy && !healthCheck.skipCheck) {
        logError(
          "syncPriceToPricingWorker: Pricing Worker is not healthy, skipping price sync",
          null,
          {
            skuId,
            healthCheck,
            reason:
              "Health check failed - Pricing Worker may be down or unreachable",
          },
        );
      ...
      const pricingPromise = pricingWorker
        .fetch(pricingRequest)
        .then(async (response) => {
          ...
          if (!response.ok) {
            logError(
              "syncPriceToPricingWorker: Failed to sync price (service binding)",
              null,
              {
                skuId,
      ...
      const pricingWorkerUrl = (
        env.PRICING_WORKER_URL ||
        "https://w2-pricing-worker.vg-firmly.workers.dev"
      ).replace(/\/$/, "");
      const catalogWorkerUrl = (
      ...
      if (!env.PRICING_SERVICE_TOKEN) {
        logError(
          "syncPriceToPricingWorker: PRICING_SERVICE_TOKEN not configured",
          null,
          { skuId },
        );
        return Promise.resolve();
      ...
      const healthCheck = await checkPricingWorkerHealthHTTP(
        pricingWorkerUrl,
        env,
      );
      ...
      return fetch(pricingEndpoint, {
        ...
        .then(async (response) => {
          ...
          if (!response.ok) {
            logError(
              "syncPriceToPricingWorker: Failed to sync price (HTTP fallback)",
              null,
              {
                skuId,
                status: response.status,
        ...
        .catch((err) => {
          logError("syncPriceToPricingWorker: HTTP fetch error", err, {
            skuId,
          });
        });
    }
  } catch (err) {
    logError("syncPriceToPricingWorker: Error setting up price sync", err, {
      skuId,
    });
    return Promise.resolve();
  }
  ...
  try {
    ...
    if (productData.title) {
      ...
      slug = await generateUniqueSlug(
        baseSlug,
        (s, excludeId) => slugExists(s, excludeId, env),
        null,
      );
    }
    ...
    const defaultImageUrl =
      "https://cdni.iconscout.com/illustration/free/thumb/free-error-404-not-found-illustration-svg-download-png-12308353.png";
    ...
    const hasImage =
      ...
      (productData.media &&
        typeof productData.media === "object" &&
        productData.media.image_url) ||
      (productData.media &&
        typeof productData.media === "object" &&
        productData.media.product_images);
    ...
    const productStockQuantity =
      productData.inventory?.stock_quantity ||
    ...
    if (productData.skus && Array.isArray(productData.skus)) {
      for (const skuData of productData.skus) {
        ...
        const skuStockQuantity =
          skuData.stock_quantity !== undefined
            ? skuData.stock_quantity
            : skuData.quantity !== undefined
              ? skuData.quantity
...
export async function updateProductService(productId, updates, userId, env) {
  try {
    ...
    const productStockQuantity =
      updates.inventory?.stock_quantity ||
 ...
 */
async function syncStockToInventoryWorker(
  skuId,
  productId,
  skuCode,
  quantity = 0,
  ...
  try {
    ...
    if (inventoryWorker) {
      ...
      const inventoryPromise = inventoryWorker
        ...
        .then(async (response) => {
          ...
          if (!response.ok) {
            logError(
              "syncStockToInventoryWorker: Failed to sync stock (service binding)",
              null,
              {
                skuId,
                status: response.status,
    ...
    logError("syncStockToInventoryWorker: Error setting up stock sync", err, {
      skuId,
    });
    return Promise.resolve();
  }
...
export async function createSkuService(skuData, userId, env, ctx = null) {
  try {
    ...
    const stockQuantity =
      skuData.stock_quantity !== undefined && skuData.stock_quantity !== null
        ? skuData.stock_quantity
    ...
    const priceSyncPromise = syncPriceToPricingWorker(
      skuId,
      skuData.product_id,
      skuCode,
      priceFields,
    ...
    // Use ctx.waitUntil() to ensure the async fetch completes even after response is sent
    const stockSyncPromise = syncStockToInventoryWorker(
      skuId,
      skuData.product_id,
      skuCode,
      stockQuantity,
 ...
 * Update SKU (admin)
 */
export async function updateSkuService(
  skuId,
  updates,
  productId,
  userId,
