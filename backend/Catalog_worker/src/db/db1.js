...
function consolidateProductFields(product) {
  ...
  if (consolidated.metadata && typeof consolidated.metadata === "object") {
    if (
      consolidated.metadata.category !== undefined &&
      consolidated.category === undefined
    ) {
  ...
  if (
    ...
    metadata.identifiers = {
      ...
      model_number:
        consolidated.model_number || existingMetadata.identifiers?.model_number,
  ...
  if (
    consolidated.weight !== undefined ||
    consolidated.weight_unit !== undefined ||
    consolidated.length !== undefined ||
    consolidated.width !== undefined ||
    consolidated.height !== undefined ||
    consolidated.dimensions_unit !== undefined ||
    consolidated.shipping_weight !== undefined ||
    consolidated.shipping_class !== undefined
  ) {
    attributes.dimensions = {
      ...
      weight:
        consolidated.weight !== undefined
          ? consolidated.weight
          : existingAttributes.dimensions?.weight,
      weight_unit:
        consolidated.weight_unit !== undefined
          ? consolidated.weight_unit
          : existingAttributes.dimensions?.weight_unit || "g",
      length:
        consolidated.length !== undefined
          ? consolidated.length
          : existingAttributes.dimensions?.length,
      width:
        consolidated.width !== undefined
          ? consolidated.width
          : existingAttributes.dimensions?.width,
      height:
        consolidated.height !== undefined
          ? consolidated.height
          : existingAttributes.dimensions?.height,
      dimensions_unit:
        consolidated.dimensions_unit !== undefined
          ? consolidated.dimensions_unit
          : existingAttributes.dimensions?.dimensions_unit || "cm",
      shipping_weight:
        consolidated.shipping_weight !== undefined
          ? consolidated.shipping_weight
          : existingAttributes.dimensions?.shipping_weight,
      shipping_class:
        consolidated.shipping_class !== undefined
          ? consolidated.shipping_class
          : existingAttributes.dimensions?.shipping_class,
    };
...
export async function slugExists(slug, excludeProductId = null, env) {
  try {
    ...
    let sql =
      "SELECT COUNT(*) as count FROM products WHERE slug = ? AND deleted = 0";
    const bindings = [slug];
...
export async function getProducts(
  { category, search, page = 1, limit = 20, featured, status = "active" },
  env,
) {
  try {
...
export async function createProduct(product, env) {
  try {
    ...
    if (!consolidated.created_at)
      consolidated.created_at = new Date().toISOString();
    if (!consolidated.updated_at)
    ...
    await env.CATALOG_DB.prepare(sql)
      .bind(...values)
      .run();

    logger("product.created", {
      productId: stringified.product_id,
      title: stringified.title,
    });
    return stringified;
  } catch (err) {
    logError("createProduct: Database error", err, {
...
export async function updateProduct(productId, updates, env) {
  try {
    ...
    logError("updateProduct: Database error", err, {
      productId,
      error: err.message,
    });
    throw err;
...
export async function deleteProduct(productId, env) {
  try {
    await env.CATALOG_DB.prepare(
      "UPDATE products SET deleted = 1, deleted_at = ? WHERE product_id = ?",
    )
...
export async function getSkusByAttributes(filters = {}, productId = null, env) {
  try {
    ...
    logError("getSkusByAttributes: Database error", err, {
      filters,
      productId,
    });
    throw err;
 ...
 */
export async function searchSkusByAttribute(
  attributeKey,
  searchValue,
  productId = null,
  env,
  ...
  try {
    ...
    logError("searchSkusByAttribute: Database error", err, {
      attributeKey,
      searchValue,
      productId,
    });
 ...
 */
export async function getSkuByAttribute(
  attributeKey,
  attributeValue,
  productId = null,
  env,
  ...
  try {
    ...
    logError("getSkuByAttribute: Database error", err, {
      attributeKey,
      attributeValue,
      productId,
    });
