...
describe("Crypto Utils", () => {
  ...
  describe("hashPassword", () => {
    ...
    it("should use pepper when provided", async () => {
      ...
      const hash2 = await crypto.hashPassword(
        password,
        salt,
        "different-pepper",
      );
  ...
  describe("decryptAESGCM", () => {
    ...
    it("should return null for wrong key", async () => {
      ...
      const wrongKey = Buffer.from(
        "wrong-encryption-key-32-bytes-long!!",
      ).toString("base64");
  ...
  describe("hashPassword error paths", () => {
    it("should handle errors when password is too short", async () => {
      const salt = crypto.genSalt();
      try {
        await crypto.hashPassword("", salt);
        expect.fail("Should have thrown error");
    ...
    it("should handle errors when salt is invalid", async () => {
      try {
        await crypto.hashPassword("password", "invalid-salt!!!");
        expect.fail("Should have thrown error");
      } catch (err) {
        // Base64 decode error or salt validation error
        expect(err).to.exist;
      }
    });
  });

  describe("decryptAESGCM error paths", () => {
    const validKey = "8CjUgdcnn2YG4sxY7aX/q9vyyx4eHRyIzZ+vLgVmKYg=";
    ...
    it("should handle corrupted encrypted data", async () => {
      ...
      const decrypted = await crypto.decryptAESGCM(corrupted, validKey);
      expect(decrypted).to.be.null;
    });
  });
});
