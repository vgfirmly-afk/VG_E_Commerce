...
export async function register(
  { name, email, password, phoneNumber, address, dateOfBirth, ssn, fullName },
  env,
) {
  try {
    // check unique
    ...
    const hashed = await hashPassword(
      password,
      salt,
      env.PEPPER || env.PEPPER_SECRET,
    ); // pass pepper from env binding
    const userId = uuidv4();
    ...
    if (hasPIIData) {
      if (!encryptionKey) {
        logError(
          "authService.register: PII data provided but PII_ENCRYPTION_KEY not set. Registration rejected for security.",
          null,
          { hasPIIData: true },
        );
        return {
          ...
          message: "PII_ENCRYPTION_KEY must be set to store PII data",
        };
      }

      try {
        logInfo("authService.register: Encrypting PII data...", {
          hasPIIData: true,
        });
        if (phoneNumber) {
          phoneNumberEncrypted = await encryptAESGCM(
            phoneNumber,
            encryptionKey,
          );
          if (!phoneNumberEncrypted) {
            logError(
              "authService.register: Failed to encrypt phoneNumber",
              null,
              { field: "phoneNumber" },
            );
            throw new Error("Encryption failed for phoneNumber");
          }
          logInfo("authService.register: phoneNumber encrypted successfully", {
            field: "phoneNumber",
          });
        }
        if (address) {
          ...
          if (!addressEncrypted) {
            logError("authService.register: Failed to encrypt address", null, {
              field: "address",
            });
            throw new Error("Encryption failed for address");
          }
          logInfo("authService.register: address encrypted successfully", {
            field: "address",
          });
        }
        if (dateOfBirth) {
          dateOfBirthEncrypted = await encryptAESGCM(
            dateOfBirth,
            encryptionKey,
          );
          if (!dateOfBirthEncrypted) {
            logError(
              "authService.register: Failed to encrypt dateOfBirth",
              null,
              { field: "dateOfBirth" },
            );
            throw new Error("Encryption failed for dateOfBirth");
          }
          logInfo("authService.register: dateOfBirth encrypted successfully", {
            field: "dateOfBirth",
          });
        }
        if (ssn) {
          ...
          if (!ssnEncrypted) {
            logError("authService.register: Failed to encrypt ssn", null, {
              field: "ssn",
            });
            throw new Error("Encryption failed for ssn");
          }
          logInfo("authService.register: ssn encrypted successfully", {
            field: "ssn",
          });
        }
        if (fullName) {
          ...
          if (!fullNameEncrypted) {
            logError("authService.register: Failed to encrypt fullName", null, {
              field: "fullName",
            });
            throw new Error("Encryption failed for fullName");
          }
          logInfo("authService.register: fullName encrypted successfully", {
            field: "fullName",
          });
        }
        ...
        logInfo("authService.register: All PII data encrypted successfully", {
          encryptedFields,
        });
      } catch (encryptErr) {
        logError("authService.register: Encryption error", encryptErr, {
          hasPIIData: true,
        });
        return {
          ok: false,
          ...
          message:
            "Failed to encrypt PII data. Registration rejected for security.",
        };
      }
    }
...
export async function rotateRefreshToken({ refreshToken }, env) {
  try {
    ...
    if (!row || row.revoked)
      return { ok: false, error: "invalid_refresh_token", status: 401 };
    if (new Date(row.expires_at) < new Date())
      return { ok: false, error: "expired_refresh_token", status: 401 };
    ...
    const accessToken = await signJWT(
      ...
      { env, expiresIn: ACCESS_TOKEN_EXPIRY },
    );

    return { ok: true, userId: user.id, accessToken, refreshToken: newToken };
  } catch (err) {
    logError("authService.rotateRefreshToken error", err, {
...
export async function revokeRefreshToken({ refreshToken, accessToken }, env) {
  try {
    ...
    logError("authService.revokeRefreshToken error", err, {
  ...
  try {
    ...
    if (encryptionKey) {
      ...
      if (user.phone_number_encrypted) {
        decryptedUser.phone_number = await decryptAESGCM(
          user.phone_number_encrypted,
          encryptionKey,
        );
      }
      if (user.address_encrypted) {
        decryptedUser.address = await decryptAESGCM(
          user.address_encrypted,
          encryptionKey,
        );
      }
      if (user.date_of_birth_encrypted) {
        decryptedUser.date_of_birth = await decryptAESGCM(
          user.date_of_birth_encrypted,
          encryptionKey,
        );
      }
      if (user.ssn_encrypted) {
        decryptedUser.ssn = await decryptAESGCM(
          user.ssn_encrypted,
          encryptionKey,
        );
      }
      if (user.full_name_encrypted) {
        decryptedUser.full_name = await decryptAESGCM(
