...
export async function getUserByEmail(email, env) {
  const res = await env.AUTH_DB.prepare("SELECT * FROM users WHERE email = ?")
    .bind(email)
    .all();
...
export async function createUser(user, env) {
  ...
  const hasPIIFields =
    user.phone_number_encrypted !== undefined ||
...
export async function getUserByIddb(id, env) {
  const res = await env.AUTH_DB.prepare("SELECT * FROM users WHERE id = ?")
    .bind(id)
    .all();
  if (!res || !res.results || res.results.length === 0) return null;
  return res.results[0];
...
export async function addRevokedToken(jti, userId, expiresAt, env) {
  ...
  await env.AUTH_DB.prepare(sql)
    .bind(jti, userId, expiresAt, new Date().toISOString())
    .run();
}
...
export async function isTokenRevoked(jti, env) {
  try {
    ...
    logWarn("isTokenRevoked: Error checking revocation, assuming not revoked", {
      error: err.message,
      jti,
    });
    return false;
...
export async function cleanupExpiredRevokedTokens(env) {
  ...
  await env.AUTH_DB.prepare("DELETE FROM revoked_tokens WHERE expires_at < ?")
    .bind(now)
    .run();
}
