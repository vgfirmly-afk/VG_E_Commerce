...
async function getProductFromCatalogWorker(productId, env) {
  try {
    ...
    if (!catalogWorker) {
      logWarn('getProductFromCatalogWorker: CATALOG_WORKER binding not available', { productId });
      return null;
    }
    ...
    if (!response.ok) {
      logWarn('getProductFromCatalogWorker: Failed to get product', { productId, status: response.status });
      return null;
    }
    
    return await response.json();
  } catch (err) {
    logWarn('getProductFromCatalogWorker: Error', { productId, error: err.message });
    return null;
  }
}
...
export async function createFulfillmentFromWebhook(webhookData, env) {
  try {
    ...
    try {
      checkoutSession = await getCheckoutSessionFromCheckoutWorker(checkout_session_id, env);
    } catch (err) {
      logWarn('createFulfillmentFromWebhook: Could not verify checkout session', { checkout_session_id, error: err.message });
    ...
    for (const item of order_data.items || []) {
      const orderItem = await createOrderItem(order.order_id, {
        sku_id: item.sku_id,
    ...
    logError('createFulfillmentFromWebhook: Service error', err, { webhookData });
    throw err;
  }
}
 ...
 * Get orders for user
 */
export async function getUserOrders(userId, guestSessionId, limit = 50, offset = 0, env) {
  try {
    let orders = [];
    ...
    if (userId) {
      ...
      orders = await getOrdersByGuestSessionId(guestSessionId, limit, offset, env);
    } else {
    ...
    const enrichedOrders = await Promise.all(orders.map(async (order) => {
      ...
      const shippingTracking = await getShippingTrackingByOrderId(order.order_id, env);
 ...
 */
export async function updateFulfillmentStatusService(orderId, status, notes, updatedBy, env) {
  try {
    const order = await getOrder(orderId, env);
    if (!order) {
    ...
    logError('updateFulfillmentStatusService: Service error', err, { orderId, status });
    throw err;
  }
}
...
export async function addShippingTrackingService(orderId, trackingData, env) {
  try {
    ...
    if (order.status !== 'shipped' && order.status !== 'delivered') {
      ...
      await createFulfillmentStatus(orderId, 'shipped', 'Shipping tracking added', 'system', env);
    }
    
    logger('shipping_tracking.added', { orderId, trackingId: tracking.tracking_id });
    return tracking;
  } catch (err) {
    logError('addShippingTrackingService: Service error', err, { orderId, trackingData });
    throw err;
  }
}
...
export async function updateShippingTrackingService(trackingId, updates, env) {
  try {
    const { getShippingTracking, updateShippingTracking } = await import('../db/db1.js');
    ...
    if (updates.status === 'delivered' && updates.actual_delivery_date) {
      await updateOrder(tracking.order_id, {
        status: 'delivered',
    ...
    logError('updateShippingTrackingService: Service error', err, { trackingId, updates });
    throw err;
  }
}
