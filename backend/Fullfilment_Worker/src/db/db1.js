 ...
 */
export async function getOrdersByGuestSessionId(
  guestSessionId,
  limit = 50,
  offset = 0,
  env,
  ...
  try {
    ...
    `,
      ...
      .bind(guestSessionId, limit, offset)
      .all();

    return orders.results || [];
  } catch (err) {
    logError("getOrdersByGuestSessionId: Database error", err, {
      guestSessionId,
    });
    throw err;
  }
...
export async function createOrderItem(orderId, itemData, env) {
  try {
    ...
    const { sku_id, product_id, quantity, unit_price, subtotal } = itemData;
 ...
 */
export async function createFulfillmentStatus(
  orderId,
  status,
  notes,
  updatedBy,
  ...
  try {
    ...
    logError("createFulfillmentStatus: Database error", err, {
      orderId,
      status,
    });
    throw err;
...
export async function createShippingTracking(orderId, trackingData, env) {
  try {
    ...
    logger("shipping_tracking.created", {
      trackingId,
      orderId,
      tracking_number,
    });
    ...
    logError("createShippingTracking: Database error", err, {
      orderId,
      trackingData,
    });
    throw err;
...
export async function updateShippingTracking(trackingId, updates, env) {
  try {
    ...
    logError("updateShippingTracking: Database error", err, {
      trackingId,
      updates,
    });
    throw err;
