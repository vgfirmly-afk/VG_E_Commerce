...
export const updateShippingTrackingSchema = Joi.object({
  status: Joi.string().valid('pending', 'in_transit', 'out_for_delivery', 'delivered', 'exception').optional(),
  estimated_delivery_date: Joi.string().optional(),
...
export function validateCreateFulfillmentWebhook(data) {
  return createFulfillmentWebhookSchema.validate(data, { abortEarly: false, stripUnknown: true });
}

export function validateUpdateFulfillmentStatus(data) {
  return updateFulfillmentStatusSchema.validate(data, { abortEarly: false, stripUnknown: true });
}

export function validateAddShippingTracking(data) {
  return addShippingTrackingSchema.validate(data, { abortEarly: false, stripUnknown: true });
}

export function validateUpdateShippingTracking(data) {
  return updateShippingTrackingSchema.validate(data, { abortEarly: false, stripUnknown: true });
}
