...
import { validateOrderId, validateCreateFulfillmentWebhook, validateUpdateFulfillmentStatus, validateAddShippingTracking, validateUpdateShippingTracking } from '../utils/validators.js';
...
export async function handleFulfillmentWebhook(request, env) {
  try {
    ...
    const result = await fulfillmentService.createFulfillmentFromWebhook(value, env);
...
export async function getUserOrders(request, env) {
  try {
    ...
    const orders = await fulfillmentService.getUserOrders(userId, guestSessionId, limit, offset, env);
    
    return new Response(
      JSON.stringify({ orders, count: orders.length }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  } catch (err) {
...
export async function addShippingTracking(request, env) {
  try {
    ...
    const tracking = await fulfillmentService.addShippingTrackingService(orderId, value, env);
...
export async function updateShippingTracking(request, env) {
  try {
    ...
    const tracking = await fulfillmentService.updateShippingTrackingService(trackingId, value, env);
