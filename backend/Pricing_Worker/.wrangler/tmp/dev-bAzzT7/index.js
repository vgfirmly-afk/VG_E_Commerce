...
var init_logger = __esm({
  "src/utils/logger.js"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    import_api23 = __toESM(require_src(), 1);
    spanLogs = /* @__PURE__ */ new WeakMap();
...
async function initializeSkuPrice(skuData, env2) {
  try {
    const {
      sku_id,
      product_id,
    ...
    const existing = await getSkuPrice(sku_id, env2);
    if (existing) {
    ...
    await env2.PRICING_DB.prepare(sql).bind(
      sku_id,
      product_id,
      sku_code,
      price,
      // Use provided price, not hardcoded 0.00!
      currency,
      // Use provided currency, not hardcoded 'USD'!
      sale_price,
      compare_at_price,
      cost_price,
      "active",
    ...
    await createPriceHistory({
      ...
      sku_code,
      price,
      // Use actual price, not 0.00!
      currency,
      // Use actual currency, not 'USD'!
      change_type: "create",
      reason,
      changed_by: "system"
    }, env2);
    ...
    const inserted = await getSkuPrice(sku_id, env2);
...
async function getProductPrices(productId, env2) {
  try {
    const res = await env2.PRICING_DB.prepare(
...
async function updateSkuPrice(skuId, priceData, userId, env2) {
  try {
    const currentPrice = await getSkuPrice(skuId, env2);
    if (!currentPrice) {
    ...
    const fields = Object.keys(updates).filter(
      (key) => key !== "sku_id" && key !== "product_id" && key !== "sku_code"
    );
    if (fields.length === 0) {
      return currentPrice;
    }
    ...
    values.push(now, skuId);
    const sql = `UPDATE sku_prices SET ${setClause}, updated_at = ? WHERE sku_id = ? AND status = ?`;
    values.push("active");
    await env2.PRICING_DB.prepare(sql).bind(...values).run();
    await createPriceHistory({
      sku_id: skuId,
      product_id: currentPrice.product_id,
      sku_code: currentPrice.sku_code,
      price: updates.price !== void 0 ? updates.price : currentPrice.price,
      currency: updates.currency || currentPrice.currency,
      sale_price: updates.sale_price !== void 0 ? updates.sale_price : currentPrice.sale_price,
      compare_at_price: updates.compare_at_price !== void 0 ? updates.compare_at_price : currentPrice.compare_at_price,
      cost_price: updates.cost_price !== void 0 ? updates.cost_price : currentPrice.cost_price,
      change_type: "update",
      reason: updates.reason || "Price update",
      changed_by: userId || "system"
    }, env2);
    logger("price.updated", { skuId, price: updates.price });
    return await getSkuPrice(skuId, env2);
  } catch (err) {
    logError("updateSkuPrice: Database error", err, { skuId, error: err.message });
    throw err;
  }
}
async function deleteSkuPrice(skuId, userId, env2) {
  try {
    const currentPrice = await getSkuPrice(skuId, env2);
    if (!currentPrice) {
      throw new Error("SKU price not found");
...
async function createPriceHistory(historyData, env2) {
  try {
    const historyId = historyData.history_id || v4_default();
    const now = (/* @__PURE__ */ new Date()).toISOString();
...
async function getPriceHistory(skuId, { page = 1, limit = 20 }, env2) {
  try {
...
async function getPromotionCode(code, env2) {
  try {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const res = await env2.PRICING_DB.prepare(
...
function calculateEffectivePrice(priceData) {
  if (!priceData) return null;
  if (priceData.sale_price !== null && priceData.sale_price !== void 0) {
    return {
...
var init_db1 = __esm({
  "src/db/db1.js"() {
    ...
    init_esm_browser();
    init_logger();
    __name(getSkuPrice, "getSkuPrice");
    __name(initializeSkuPrice, "initializeSkuPrice");
    __name(getSkuPrices, "getSkuPrices");
    __name(getProductPrices, "getProductPrices");
    __name(updateSkuPrice, "updateSkuPrice");
    __name(deleteSkuPrice, "deleteSkuPrice");
    __name(createPriceHistory, "createPriceHistory");
    __name(getPriceHistory, "getPriceHistory");
    __name(getPromotionCode, "getPromotionCode");
    __name(calculateEffectivePrice, "calculateEffectivePrice");
  }
});
...
// .wrangler/tmp/bundle-n6fbgc/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-n6fbgc/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
function resolveConfig(env2 = {}) {
  const SERVICE_NAME = env2.SERVICE_NAME || "Pricing_Worker_Service";
...
// src/handlers/pricingHandlers.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
init_db1();
init_logger();
async function getPrice(skuId, env2) {
  try {
    const priceData = await getSkuPrice(skuId, env2);
    if (!priceData) {
      return null;
    }
...
async function getProductPricing(productId, env2) {
  try {
    const prices = await getProductPrices(productId, env2);
...
__name(getProductPricing, "getProductPricing");
async function calculateGrandTotal(items, promotionCode = null, currency = "USD", env2) {
  try {
    const skuIds = items.map((item) => item.sku_id);
    const prices = await getSkuPrices(skuIds, env2);
    ...
    let promotion = null;
    if (promotionCode) {
      const promo = await getPromotionCode(promotionCode, env2);
      if (promo && promo.status === "active") {
        let applicable = true;
        if (promo.applicable_skus) {
          try {
            const applicableSkus = typeof promo.applicable_skus === "string" ? JSON.parse(promo.applicable_skus) : promo.applicable_skus;
...
__name(calculateGrandTotal, "calculateGrandTotal");
async function updatePrice(skuId, priceData, userId, env2) {
  try {
    const updated = await updateSkuPrice(skuId, priceData, userId, env2);
...
async function deletePrice(skuId, userId, env2) {
  try {
    await deleteSkuPrice(skuId, userId, env2);
    logger("price.deleted", { skuId });
    return true;
  } catch (err) {
...
__name(deletePrice, "deletePrice");
async function getHistory(skuId, query, env2) {
  try {
    const history = await getPriceHistory(skuId, query, env2);
...
var skuPriceSchema = import_joi.default.object({
  sku_id: import_joi.default.string().min(1).max(100).required(),
  product_id: import_joi.default.string().min(1).max(100).required(),
  sku_code: import_joi.default.string().min(1).max(100).required(),
  price: import_joi.default.number().min(0).required(),
  currency: import_joi.default.string().length(3).default("USD"),
  sale_price: import_joi.default.number().min(0).allow(null),
  compare_at_price: import_joi.default.number().min(0).allow(null),
  cost_price: import_joi.default.number().min(0).allow(null),
  status: import_joi.default.string().valid("active", "inactive").default("active"),
  valid_from: import_joi.default.string().isoDate().default(() => (/* @__PURE__ */ new Date()).toISOString()),
  valid_to: import_joi.default.string().isoDate().allow(null)
});
var skuPriceUpdateSchema = import_joi.default.object({
  price: import_joi.default.number().min(0).optional(),
  currency: import_joi.default.string().length(3).optional(),
  sale_price: import_joi.default.number().min(0).allow(null).optional(),
  compare_at_price: import_joi.default.number().min(0).allow(null).optional(),
  cost_price: import_joi.default.number().min(0).allow(null).optional(),
  status: import_joi.default.string().valid("active", "inactive").optional(),
  valid_from: import_joi.default.string().isoDate().optional(),
  valid_to: import_joi.default.string().isoDate().allow(null).optional(),
  reason: import_joi.default.string().max(500).allow(null, "").optional()
  // Reason for price change
});
var calculateTotalSchema = import_joi.default.object({
  ...
  promotion_code: import_joi.default.string().max(50).allow(null, "").optional(),
  currency: import_joi.default.string().length(3).default("USD").optional()
});
var priceHistoryQuerySchema = import_joi.default.object({
  sku_id: import_joi.default.string().min(1).max(100).required(),
  page: import_joi.default.number().integer().min(1).max(1e4).default(1),
  limit: import_joi.default.number().integer().min(1).max(100).default(20)
});
var skuIdSchema = import_joi.default.string().min(1).max(100).required().messages({
  "string.empty": "SKU ID is required",
  "any.required": "SKU ID is required"
});
var promotionCodeSchema = import_joi.default.string().min(1).max(50).required().messages({
  "string.empty": "Promotion code is required",
  "any.required": "Promotion code is required"
});
function validateSkuPriceUpdate(data) {
...
__name(validateSkuPriceUpdate, "validateSkuPriceUpdate");
function validateCalculateTotal(data) {
  return calculateTotalSchema.validate(data, { abortEarly: false, stripUnknown: true });
}
__name(validateCalculateTotal, "validateCalculateTotal");
function validatePriceHistoryQuery(params) {
  return priceHistoryQuerySchema.validate(params, { abortEarly: false, stripUnknown: true });
}
__name(validatePriceHistoryQuery, "validatePriceHistoryQuery");
function validateSkuId(skuId) {
  return skuIdSchema.validate(skuId);
}
__name(validateSkuId, "validateSkuId");

// src/handlers/pricingHandlers.js
init_logger();
async function getPrice2(request, env2) {
  try {
    ...
    const price = await getPrice(skuId, env2);
    if (!price) {
      return new Response(
...
__name(getPrice2, "getPrice");
async function getProductPrices2(request, env2) {
  try {
    ...
    const prices = await getProductPricing(productId, env2);
    return new Response(
...
__name(getProductPrices2, "getProductPrices");
async function calculateTotal(request, env2) {
  try {
    const body = await request.json();
    const { error: error3, value } = validateCalculateTotal(body);
    if (error3) {
      return new Response(
        JSON.stringify({
          error: "validation_error",
          message: "Invalid request data",
          details: error3.details.map((d) => ({
            path: d.path.join("."),
            message: d.message,
            type: d.type
          }))
...
async function getHistory2(request, env2) {
  try {
    const skuId = request.params?.sku_id;
    const { error: idError } = validateSkuId(skuId);
    if (idError) {
    ...
    const history = await getHistory(skuId, queryParams, env2);
    return new Response(
      JSON.stringify({
        sku_id: skuId,
        history,
...
__name(getHistory2, "getHistory");
async function initializePrice(request, env2) {
  try {
    const skuId = request.params?.sku_id;
    const { error: idError } = validateSkuId(skuId);
    if (idError) {
    ...
    const { error: error3, value } = validateSkuPriceUpdate(body);
    if (error3) {
      console.error("[Pricing Handler] Validation error:", error3);
      return new Response(
        JSON.stringify({
          error: "validation_error",
    ...
    console.log("[Pricing Handler] Validated price data:", {
      ...
      hasPrice: value.price !== void 0,
      price: value.price
    });
    const { initializeSkuPrice: initializeSkuPrice2, updateSkuPrice: updateSkuPrice2, getSkuPrice: getSkuPrice2 } = await Promise.resolve().then(() => (init_db1(), db1_exports));
    const existing = await getSkuPrice2(skuId, env2);
    let price;
    if (existing) {
      console.log("[Pricing Handler] Price exists, updating:", { skuId, existingPrice: existing.price });
      price = await updateSkuPrice2(skuId, value, userId, env2);
    } else {
      const initData = {
        sku_id: skuId,
        product_id: body.product_id || "",
        sku_code: body.sku_code || "",
        price: value.price !== void 0 ? value.price : body.price !== void 0 ? body.price : 0,
        currency: value.currency || body.currency || "USD",
        sale_price: value.sale_price !== void 0 ? value.sale_price : body.sale_price !== void 0 ? body.sale_price : null,
        compare_at_price: value.compare_at_price !== void 0 ? value.compare_at_price : body.compare_at_price !== void 0 ? body.compare_at_price : null,
        cost_price: value.cost_price !== void 0 ? value.cost_price : body.cost_price !== void 0 ? body.cost_price : null,
      ...
      price = await initializeSkuPrice2(initData, env2);
    }
...
__name(initializePrice, "initializePrice");
async function updatePrice2(request, env2) {
  try {
    const skuId = request.params?.sku_id;
    const { error: idError } = validateSkuId(skuId);
    if (idError) {
    ...
    const price = await updatePrice(skuId, value, userId, env2);
    return new Response(
      JSON.stringify(price),
      { status: 200, headers: { "Content-Type": "application/json" } }
    );
...
__name(updatePrice2, "updatePrice");
async function deletePrice2(request, env2) {
  try {
    const skuId = request.params?.sku_id;
    const { error: idError } = validateSkuId(skuId);
    if (idError) {
    ...
    const userId = request.user?.userId || "system";
    await deletePrice(skuId, userId, env2);
    return new Response(
...
__name(deletePrice2, "deletePrice");
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var getCatalogWorkerUrl = /* @__PURE__ */ __name((env2) => {
  return env2.CATALOG_WORKER_URL || "https://w2-catalog-worker.vg-firmly.workers.dev";
}, "getCatalogWorkerUrl");
var getWhitelistedUrls = /* @__PURE__ */ __name((env2) => {
  const catalogUrl = getCatalogWorkerUrl(env2);
  return [catalogUrl.replace(/\/$/, "")];
}, "getWhitelistedUrls");

// src/middleware/adminAuth.js
function validateCatalogWorkerSource(request, env2) {
  try {
    const sourceHeader = request.headers.get("X-Source");
    if (sourceHeader === "catalog-worker-service-binding") {
      return true;
    }
    const whitelistedUrls = getWhitelistedUrls(env2);
    if (!whitelistedUrls || whitelistedUrls.length === 0) {
      logWarn("validateCatalogWorkerSource: No whitelisted URLs configured");
      return false;
    }
    if (sourceHeader) {
      const sourceUrl = sourceHeader.replace(/\/$/, "");
...
__name(validateCatalogWorkerSource, "validateCatalogWorkerSource");
function validateServiceToken(token, env2) {
  if (!env2.PRICING_SERVICE_TOKEN) {
    logWarn("validateServiceToken: PRICING_SERVICE_TOKEN not configured");
    return false;
  }
  return token === env2.PRICING_SERVICE_TOKEN;
}
__name(validateServiceToken, "validateServiceToken");
async function requireAdmin(request, env2) {
  try {
    ...
    const isFromCatalogWorker = validateCatalogWorkerSource(request, env2);
    if (isFromCatalogWorker) {
      if (validateServiceToken(token, env2)) {
        return {
          ok: true,
          user: {
            userId: "catalog_worker_service",
...
// src/routers/pricing.js
var router = e();
router.get("/_/health", async (request, env2, ctx) => {
  const url = new URL(request.url);
...
router.get(
  ...
  async (request, env2, ctx) => getProductPrices2(request, request.env || env2)
);
router.get(
  "/api/v1/prices/:sku_id/history",
  async (request, env2, ctx) => getHistory2(request, request.env || env2)
);
router.post(
  "/api/v1/prices/:sku_id",
  async (request, env2, ctx) => {
    ...
    return initializePrice(request, request.env || env2);
  }
);
router.get(
  "/api/v1/prices/:sku_id",
  async (request, env2, ctx) => getPrice2(request, request.env || env2)
);
router.post(
  "/api/v1/calculate-total",
  async (request, env2, ctx) => calculateTotal(request, request.env || env2)
);
router.put(
  "/api/v1/prices/:sku_id",
  async (request, env2, ctx) => {
    ...
    return updatePrice2(request, request.env || env2);
  }
);
router.delete(
  "/api/v1/prices/:sku_id",
  async (request, env2, ctx) => {
    ...
    return deletePrice2(request, request.env || env2);
  }
);
...
var pricing_default = safeRouter;
...
function withLogger(handler) {
  return async function(request, env2, ctx) {
    ...
    let res;
    try {
      res = await handler(request, env2, ctx);
    } catch (err) {
    ...
    try {
      ...
      console.error("[Logger] Failed to set trace headers:", e2);
...
async function baseFetch(request, env2, ctx) {
  try {
    request.env = env2;
    const response = await pricing_default.handle(request, env2, ctx);
    if (!response || !(response instanceof Response)) {
...
var src_default = exportedHandler;
