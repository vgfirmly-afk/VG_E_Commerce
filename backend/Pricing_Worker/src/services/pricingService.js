...
export async function calculateGrandTotal(items, promotionCode = null, currency = 'USD', env) {
  try {
    ...
    let promotionError = promotionCode ? 'Promotion code not found or expired' : null;
    ...
    if (promotionCode) {
      try {
        ...
        console.log('[Pricing Service] Looking up promotion code:', normalizedCode);
        
        const promo = await getPromotionCode(normalizedCode, env);
        
        if (!promo) {
          promotionError = 'Promotion code not found or expired';
          console.log('[Pricing Service] Promotion code not found:', normalizedCode);
        } else {
          ...
          if (promo.status !== 'active') {
            ...
            console.log('[Pricing Service] Promotion not active:', promo.status);
          } else {
            ...
            if (promo.applicable_skus) {
              try {
                ...
                if (Array.isArray(applicableSkus) && applicableSkus.length > 0) {
                  ...
                  if (!applicable) {
                    failureReason = 'Promotion code does not apply to items in cart';
                ...
                console.log('[Pricing Service] Failed to parse applicable_skus, assuming all SKUs');
              }
            }
            
            // Check minimum purchase amount
            if (applicable && promo.min_purchase_amount && subtotal < promo.min_purchase_amount) {
              applicable = false;
            ...
            // Check usage limit
            if (applicable && promo.usage_limit && promo.usage_count >= promo.usage_limit) {
              applicable = false;
            ...
            if (applicable) {
              ...
              if (promo.discount_type === 'percentage') {
                ...
                console.log('[Pricing Service] Percentage discount calculated:', {
                  subtotal,
              ...
              try {
                ...
                console.log('[Pricing Service] Usage count incremented for:', normalizedCode);
              } catch (err) {
                // Log but don't fail the calculation if usage increment fails
                logError('incrementPromotionUsage: Error', err, { code: normalizedCode });
              }
            } else {
        ...
        logError('calculateGrandTotal: Error processing promotion code', err, { promotionCode });
...
export async function listPromotionCodesService(query, env) {
  try {
    ...
    logger('promotion_codes.listed', { count: result.promotions.length, page: query.page });
    return result;
  } catch (err) {
...
export async function createPromotionCodeService(promoData, userId, env) {
  try {
    ...
    logger('promotion_code.created', { promotionId: promo.promotion_id, code: promo.code });
 ...
 */
export async function updatePromotionCodeService(promotionId, updates, userId, env) {
  try {
    const promo = await updatePromotionCode(promotionId, updates, userId, env);
    logger('promotion_code.updated', { promotionId, updates: Object.keys(updates) });
    ...
    logError('updatePromotionCodeService: Error', err, { promotionId, updates });
    throw err;
  }
}
