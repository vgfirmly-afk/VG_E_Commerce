...
async function getProductNameAndSkuAttributes(productId, skuId, env) {
  try {
    ...
    if (!catalogWorker) {
      logError(
        "getProductNameAndSkuAttributes: CATALOG_WORKER binding not available",
        null,
        { productId, skuId },
      );
      return null;
    ...
    const productRequest = new Request(
      `https://catalog-worker/api/v1/products/${encodeURIComponent(productId)}`,
      {
        method: "GET",
        headers: {
    ...
    logError("getProductNameAndSkuAttributes: Error", err, {
      productId,
      skuId,
    });
    return null;
...
async function getStockFromInventory(skuId, env) {
  try {
    ...
    if (!inventoryWorker) {
      logError(
        "getStockFromInventory: INVENTORY_WORKER binding not available",
        null,
        { skuId },
      );
      return null;
...
export async function getPrice(request, env) {
  try {
    ...
    const [productInfo, stockQuantity] = await Promise.all([
      productId
        ? getProductNameAndSkuAttributes(productId, skuId, env)
...
export async function getProductPrices(request, env) {
  try {
    ...
    return new Response(JSON.stringify({ product_id: productId, prices }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err) {
...
export async function initializePrice(request, env) {
  try {
    ...
    const { initializeSkuPrice, updateSkuPrice, getSkuPrice } = await import(
      "../db/db1.js"
    );
    ...
    let price;
    if (existing) {
      console.log("[Pricing Handler] Price exists, updating:", {
        skuId,
        ...
        compare_at_price:
          value.compare_at_price !== undefined
            ...
            : body.compare_at_price !== undefined
              ...
              : null,
        cost_price:
          value.cost_price !== undefined
            ? value.cost_price
            : body.cost_price !== undefined
              ? body.cost_price
              : null,
        reason:
          body.reason || value.reason || "Price synced from Catalog Worker",
      };
...
export async function getPromotionCode(request, env) {
  try {
    ...
    const promo = await pricingService.getPromotionCodeService(
      promotionId,
      env,
    );
...
export async function createPromotionCode(request, env) {
  try {
    ...
    const promo = await pricingService.createPromotionCodeService(
      value,
      userId,
      env,
    );
...
export async function updatePromotionCode(request, env) {
  try {
    ...
    const promo = await pricingService.updatePromotionCodeService(
      promotionId,
      value,
      userId,
      env,
