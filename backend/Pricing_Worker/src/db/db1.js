...
export async function initializeSkuPrice(skuData, env) {
  try {
    ...
    await env.PRICING_DB.prepare(sql)
      .bind(
        sku_id,
        product_id,
        sku_code,
...
export async function getSkuPrices(skuIds, env) {
  try {
    ...
    const res = await env.PRICING_DB.prepare(sql)
      .bind(...skuIds, "active")
      .all();
    return res?.results || [];
  } catch (err) {
...
export async function updateSkuPrice(skuId, priceData, userId, env) {
  try {
    ...
    logError("updateSkuPrice: Database error", err, {
      skuId,
      error: err.message,
    });
    throw err;
...
export async function listPromotionCodes(
  ...
  try {
    ...
    const countRes = await env.PRICING_DB.prepare(countSql)
      .bind(...countParams)
      .first();
    ...
    logError("listPromotionCodes: Database error", err, {
      page,
      limit,
      status,
    });
  ...
  try {
    ...
    if (updateFields.applicable_skus !== undefined) {
      if (Array.isArray(updateFields.applicable_skus)) {
        updateFields.applicable_skus = JSON.stringify(
          updateFields.applicable_skus,
        );
    ...
    const fields = Object.keys(updateFields).filter(
      (key) => key !== "promotion_id" && key !== "code", // Don't allow changing ID or code
    );
    ...
    logError("updatePromotionCode: Database error", err, {
      promotionId,
      updates,
    });
    throw err;
