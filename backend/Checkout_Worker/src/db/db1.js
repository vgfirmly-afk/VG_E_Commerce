...
export async function updateCheckoutSession(sessionId, updates, env) {
  try {
    ...
    logError("updateCheckoutSession: Database error", err, {
      sessionId,
      updates,
    });
    throw err;
  ...
  try {
    ...
    `,
      ...
      .bind(
        ...
        methodData.carrier,
        methodData.base_cost || 0.0,
        methodData.cost_per_kg || 0.0,
        methodData.min_delivery_days || 3,
        methodData.max_delivery_days || 7,
        methodData.is_active !== undefined ? (methodData.is_active ? 1 : 0) : 1,
        methodData.applicable_pincodes
          ? JSON.stringify(methodData.applicable_pincodes)
          : null,
        now,
        now,
      )
...
export async function updateShippingMethod(methodId, updates, env) {
  try {
    ...
    if (updates.applicable_pincodes !== undefined) {
      ...
      bindValues.push(
        updates.applicable_pincodes
          ? JSON.stringify(updates.applicable_pincodes)
          : null,
      );
    }
    ...
    logError("updateShippingMethod: Database error", err, {
      methodId,
      updates,
    });
    throw err;
