 ...
 */
export async function updateCheckoutSessionPaymentStatus(
  sessionId,
  paymentId,
  paymentStatus,
  paymentData,
  ...
  try {
    ...
    if (paymentStatus === "captured") {
      ...
      try {
        ...
        logger("checkout.stock.released", {
          sessionId,
          reason: "payment_failed",
        });
      } catch (err) {
        logError(
          "updateCheckoutSessionPaymentStatus: Failed to release stock",
          err,
          { sessionId },
        );
      }
    ...
    logError("updateCheckoutSessionPaymentStatus: Service error", err, {
      sessionId,
      paymentStatus,
    });
    throw err;
...
export async function createSession(cartId, userId, guestSessionId, env) {
  try {
    ...
    logger("checkout.session.created", {
...
export async function setDeliveryAddress(
  ...
  try {
    ...
    logger("checkout.delivery_address.set", {
      sessionId,
...
export async function setBillingAddress(
  ...
  try {
    ...
    logger("checkout.billing_address.set", {
      sessionId,
...
export async function selectShippingMethod(sessionId, methodId, env) {
  try {
    ...
    logError("selectShippingMethod: Service error", err, {
      sessionId,
      methodId,
    });
    throw err;
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!stockCheck.available) {
        ...
        // Reserve stock in KV
        const reservation = await reserveStockInKV(
          sessionId,
          item.sku_id,
          item.quantity,
          env,
    ...
    for (const item of items) {
      const currentPrice =
        (await getPriceFromPricingWorker(item.sku_id, env)) || item.unit_price;
...
export async function updateShippingMethodService(methodId, updates, env) {
  try {
    ...
    logError("updateShippingMethodService: Service error", err, {
      methodId,
      updates,
    });
    throw err;
