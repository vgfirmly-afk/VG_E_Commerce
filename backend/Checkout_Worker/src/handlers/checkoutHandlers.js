...
export async function createSession(request, env) {
  try {
    ...
    const status =
      err.message.includes("not found") || err.message.includes("empty")
        ? 404
        : 500;
    return new Response(
...
export async function getShippingMethods(request, env) {
  try {
    ...
    return new Response(JSON.stringify({ shipping_methods: methods }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err) {
    ...
    const status =
      err.message.includes("not found") || err.message.includes("not set")
        ? 404
        : 500;
    return new Response(
      JSON.stringify({ error: "internal_error", message: err.message }),
...
export async function getSummary(request, env) {
  try {
    ...
    const status = err.message.includes("out of stock")
      ? 400
      : err.message.includes("not found") || err.message.includes("not set")
        ? 404
        : 500;
    ...
    if (idError) {
      return new Response(
        JSON.stringify({
          error: "validation_error",
          message:
            idError.details?.[0]?.message || "Invalid shipping method ID",
        }),
        { status: 400, headers: { "Content-Type": "application/json" } },
      );
    }
