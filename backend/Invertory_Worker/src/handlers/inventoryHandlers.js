...
export async function adjustStock(request, env) {
  try {
    ...
    const stock = await inventoryService.adjustStock(skuId, quantity, userId, env, reason, reservation_id);
...
export async function reserveStock(request, env) {
  try {
    ...
    const stock = await inventoryService.reserveStockForCart(skuId, quantity, reservation_id, userId, env, expires_at);
...
export async function releaseStock(request, env) {
  try {
    ...
    const stock = await inventoryService.releaseReservedStock(skuId, reservation_id, quantity, userId, env);
