...
export async function getSkuStocks(skuIds, env) {
  try {
    ...
    const res = await env.INVENTORY_DB.prepare(sql).bind(...skuIds).all();
    return res?.results || [];
  } catch (err) {
 ...
 */
export async function adjustStockQuantity(skuId, adjustmentQuantity, userId, env, reason = 'Stock adjustment', reservationId = null) {
  try {
    ...
    if (newQuantity < 0) {
      throw new Error(`Insufficient stock. Available: ${existing.available_quantity}, Requested: ${Math.abs(adjustmentQuantity)}`);
    }
    
    const now = new Date().toISOString();
    const available_quantity = Math.max(0, newQuantity - existing.reserved_quantity);
    const status = newQuantity > 0 ? (existing.status === 'inactive' ? 'inactive' : 'active') : 'out_of_stock';
    ...
    logError('adjustStockQuantity: Database error', err, { skuId, adjustmentQuantity });
    throw err;
  }
}
 ...
 */
export async function reserveStock(skuId, quantity, reservationId, userId, env, expiresAt = null) {
  try {
    const existing = await getSkuStock(skuId, env);
    if (!existing) {
      throw new Error(`Stock not found for SKU: ${skuId}`);
    ...
    if (existing.available_quantity < quantity) {
      throw new Error(`Insufficient available stock. Available: ${existing.available_quantity}, Requested: ${quantity}`);
    }
    ...
    if (updated.reserved_quantity !== newReservedQuantity) {
      throw new Error('Stock reservation failed - insufficient available stock');
    }
    ...
    logError('reserveStock: Database error', err, { skuId, quantity, reservationId });
    throw err;
  }
}
 ...
 * Uses ACID transaction
 */
export async function releaseStock(skuId, reservationId, quantity = null, userId, env) {
  try {
    // Get reservation
    const reservation = await env.INVENTORY_DB.prepare(
      'SELECT * FROM stock_reservations WHERE reservation_id = ? AND status = ?'
    ).bind(reservationId, 'active').first();
    ...
    if (reservation.sku_id !== skuId) {
      throw new Error(`Reservation ${reservationId} does not belong to SKU ${skuId}`);
    }
    
    const releaseQuantity = quantity || reservation.quantity;
    if (releaseQuantity > reservation.quantity) {
      throw new Error(`Cannot release more than reserved. Reserved: ${reservation.quantity}, Requested: ${releaseQuantity}`);
    }
    ...
    const now = new Date().toISOString();
    const newReservedQuantity = Math.max(0, existing.reserved_quantity - releaseQuantity);
    const newAvailableQuantity = existing.quantity - newReservedQuantity;
    ...
    logError('releaseStock: Database error', err, { skuId, reservationId, quantity });
    throw err;
  }
}
