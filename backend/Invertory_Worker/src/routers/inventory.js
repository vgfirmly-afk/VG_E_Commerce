...
router.get('/api/v1/stock/:sku_id', async (request, env, ctx) => {
  try {
    ...
    console.error('[Inventory Router] Error in GET /api/v1/stock/:sku_id:', err);
    return new Response(
      JSON.stringify({ error: 'internal_error', message: err?.message ?? String(err) }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
...
router.get('/api/v1/stock/:sku_id/history', async (request, env, ctx) => {
  try {
    ...
    console.error('[Inventory Router] Error in GET /api/v1/stock/:sku_id/history:', err);
    return new Response(
      JSON.stringify({ error: 'internal_error', message: err?.message ?? String(err) }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
...
router.post('/api/v1/check-stock', 
  async (request, env, ctx) => {
    try {
      console.log('[Inventory Router] POST /api/v1/check-stock matched (PUBLIC ENDPOINT)', { 
        method: request.method, 
        url: request.url,
      ...
      return await inventoryHandlers.checkAvailability(request, request.env || env);
    } catch (err) {
...
router.post('/api/v1/stock/check', 
  async (request, env, ctx) => {
    console.log('[Inventory Router] POST /api/v1/stock/check - redirecting to /api/v1/check-stock');
    // Return a helpful error message directing users to the new endpoint
    return new Response(
      JSON.stringify({ 
        error: 'endpoint_moved', 
        message: 'This endpoint has been moved. Please use POST /api/v1/check-stock instead.',
        new_endpoint: '/api/v1/check-stock'
      }),
      { status: 301, headers: { 'Content-Type': 'application/json', 'Location': '/api/v1/check-stock' } }
    );
  }
);
...
router.post('/api/v1/stock/:sku_id',
  async (request, env, ctx) => {
    try {
      ...
      if (skuId === 'check') {
        console.log('[Inventory Router] POST /api/v1/stock/:sku_id matched with sku_id="check" - this should not happen, returning 404');
        return new Response(
          JSON.stringify({ 
            error: 'not_found', 
            message: 'Route not found. Use POST /api/v1/stock/check for checking stock availability.' 
          }),
      ...
      console.error('[Inventory Router] Error in POST /api/v1/stock/:sku_id:', err);
      return new Response(
        JSON.stringify({ error: 'internal_error', message: err?.message ?? String(err) }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
...
router.put('/api/v1/stock/:sku_id',
  ...
  async (request, env, ctx) => {
    try {
      ...
      console.error('[Inventory Router] Error in PUT /api/v1/stock/:sku_id:', err);
      return new Response(
        JSON.stringify({ error: 'internal_error', message: err?.message ?? String(err) }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
...
router.post('/api/v1/stock/:sku_id/adjust',
  ...
  async (request, env, ctx) => {
    try {
      ...
      console.error('[Inventory Router] Error in POST /api/v1/stock/:sku_id/adjust:', err);
      return new Response(
        JSON.stringify({ error: 'internal_error', message: err?.message ?? String(err) }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
...
router.post('/api/v1/stock/:sku_id/reserve',
  ...
  async (request, env, ctx) => {
    try {
      ...
      console.error('[Inventory Router] Error in POST /api/v1/stock/:sku_id/reserve:', err);
...
router.post('/api/v1/stock/:sku_id/release',
  ...
  async (request, env, ctx) => {
    try {
      ...
      console.error('[Inventory Router] Error in POST /api/v1/stock/:sku_id/release:', err);
      return new Response(
        JSON.stringify({ error: 'internal_error', message: err?.message ?? String(err) }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
...
router.post('/api/v1/webhooks/payment-status',
  async (request, env, ctx) => {
    try {
      ...
      console.error('[Inventory Router] Error in POST /api/v1/webhooks/payment-status:', err);
      return new Response(
        JSON.stringify({ error: 'internal_error', message: err?.message ?? String(err) }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
