...
```javascript
// In addItem() function
const reserveRequest = new Request(
  `https://inventory-worker/api/v1/stock/${skuId}/reserve`,
  {
    method: "POST",
    headers: {
...
const calculateRequest = new Request(
  `https://pricing-worker/api/v1/calculate-total`,
  {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
