...
router.post("/api/v1/cart/:cart_id/items", async (request, env, ctx) => {
  try {
    ...
    try {
      ...
      if (bodyText.includes("{{") && bodyText.includes("}}")) {
        console.error(
          "[Cart Router] Unresolved Postman variables detected in body:",
        ...
        return new Response(
          JSON.stringify({
            error: "validation_error",
            message:
              "Request body contains unresolved variables (e.g., {{skuId}}). Please ensure all Postman variables are set.",
    ...
    console.error(
      "[Cart Router] Error in POST /api/v1/cart/:cart_id/items:",
      err,
    );
    return new Response(
      JSON.stringify({
...
router.put(
  ...
  async (request, env, ctx) => {
    try {
      ...
      console.error(
        "[Cart Router] Error in PUT /api/v1/cart/:cart_id/items/:item_id:",
        err,
      );
      return new Response(
        JSON.stringify({
