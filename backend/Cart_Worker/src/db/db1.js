...
export async function addItemToCart(cartId, skuId, quantity, currency, env) {
  try {
    ...
    if (existing) {
      ...
      // Log history
      await logCartHistory(cartId, 'update_quantity', skuId, existing.quantity, newQuantity, 'system', env);
      ...
      // Log history
      await logCartHistory(cartId, 'add_item', skuId, 0, quantity, 'system', env);
...
export async function updateItemQuantity(itemId, quantity, env) {
  try {
    ...
    if (quantity === 0) {
      ...
      await env.CART_DB.prepare(
        'UPDATE carts SET updated_at = ? WHERE cart_id = ?'
      ).bind(now, item.cart_id).run();
      
      // Log history
      await logCartHistory(item.cart_id, 'remove_item', item.sku_id, item.quantity, 0, 'system', env);
      ...
      // Log history
      await logCartHistory(item.cart_id, 'update_quantity', item.sku_id, item.quantity, quantity, 'system', env);
 ...
 * Log cart history
 */
async function logCartHistory(cartId, action, skuId, quantityBefore, quantityAfter, changedBy, env) {
  try {
    const now = new Date().toISOString();
    await env.CART_DB.prepare(`
