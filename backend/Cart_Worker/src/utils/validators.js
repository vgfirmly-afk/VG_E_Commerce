...
// Cart ID schema
export const cartIdSchema = Joi.string().min(1).max(100).required().messages({
  "string.empty": "Cart ID is required",
  "any.required": "Cart ID is required",
});

// Item ID schema
export const itemIdSchema = Joi.string().min(1).max(100).required().messages({
...
export function validateAddItem(data) {
  return addItemSchema.validate(data, {
    abortEarly: false,
    stripUnknown: true,
  });
}
...
export function validateUpdateQuantity(data) {
  return updateQuantitySchema.validate(data, {
    abortEarly: false,
    stripUnknown: true,
  });
}
