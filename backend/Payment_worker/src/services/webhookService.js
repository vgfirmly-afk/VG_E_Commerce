...
import { capturePayPalOrder, getPayPalOrder } from "./paypalService.js";
...
async function notifyCheckoutWorker(payment, env) {
  try {
    ...
    if (!checkoutWorker) {
      logWarn("notifyCheckoutWorker: CHECKOUT_WORKER binding not available", {
    ...
    logError("notifyCheckoutWorker: Error", err, {
      payment_id: payment.payment_id,
    });
  }
}
...
async function notifyInventoryWorker(payment, orderItems, env) {
  try {
    ...
    if (!inventoryWorker) {
      logWarn("notifyInventoryWorker: INVENTORY_WORKER binding not available", {
        payment_id: payment.payment_id,
      });
      return;
    }
    ...
    if (!orderItems || orderItems.length === 0) {
      logWarn("notifyInventoryWorker: No order items provided", {
        payment_id: payment.payment_id,
      });
      return;
    }
    ...
    const webhookUrl =
      "https://inventory-worker/api/v1/webhooks/payment-status";
    const webhookData = {
      payment_id: payment.payment_id,
      payment_status: payment.status,
      checkout_session_id: payment.checkout_session_id,
    ...
    logError("notifyInventoryWorker: Error", err, {
      payment_id: payment.payment_id,
    });
  }
}
...
async function notifyFulfillmentWorker(payment, checkoutSession, env) {
  try {
    ...
    if (!fulfillmentWorker) {
      logWarn(
        "notifyFulfillmentWorker: FULFILLMENT_WORKER binding not available",
        { payment_id: payment.payment_id },
      );
      return;
    }
    ...
    if (!checkoutSession) {
      logWarn("notifyFulfillmentWorker: Checkout session not provided", {
        payment_id: payment.payment_id,
      });
      return;
    }
      ...
      order_data: {
        delivery_address: checkoutSession.delivery_address
          ? {
              ...
              address_line2:
                checkoutSession.delivery_address.address_line2 || null,
        ...
        billing_address: checkoutSession.billing_address
          ? {
              ...
              address_line2:
                checkoutSession.billing_address.address_line2 || null,
        ...
        estimated_delivery_date:
          checkoutSession.estimated_delivery_date || null,
      },
    };

    const response = await fulfillmentWorker.fetch(
      new Request(webhookUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      logger("webhook.fulfillment.notified", {
        payment_id: payment.payment_id,
      });
    }
  } catch (err) {
    logError("notifyFulfillmentWorker: Error", err, {
      payment_id: payment.payment_id,
    });
  }
}
...
async function getProductIdForSku(skuId, env) {
  try {
    ...
    logWarn("getProductIdForSku: Error getting product_id", {
      skuId,
      error: err.message,
    });
    return null;
...
async function handlePaymentCaptureCompleted(resource, env) {
  try {
    ...
    if (!orderId) {
      logError("handlePaymentCaptureCompleted: No order ID in resource", null, {
        resource,
      });
    ...
    if (!payment) {
      logError("handlePaymentCaptureCompleted: Payment not found", null, {
        orderId,
      });
    ...
    notifyCheckoutWorker(payment, env).catch((err) => {
      logError("notifyCheckoutWorker: Failed", err, {
        payment_id: payment.payment_id,
      });
    });
    ...
    if (checkoutSession && checkoutSession.items) {
      ...
      notifyInventoryWorker(payment, enrichedItems, env).catch((err) => {
        logError("notifyInventoryWorker: Failed", err, {
          payment_id: payment.payment_id,
        });
      });
    }
    ...
    if (checkoutSession) {
      ...
      notifyFulfillmentWorker(payment, checkoutSession, env).catch((err) => {
        logError("notifyFulfillmentWorker: Failed", err, {
          payment_id: payment.payment_id,
        });
      });
    }
