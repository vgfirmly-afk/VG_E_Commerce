...
export async function getSkuStock(skuId) {
	const response = await fetchWithCredentials(`${API_BASE}/api/v1/inventory/sku/${skuId}`, {
		method: 'GET'
	});
...
export async function getProductStock(productId) {
	const response = await fetchWithCredentials(`${API_BASE}/api/v1/inventory/product/${productId}`, {
		method: 'GET'
	});
