...
export async function createPayment(checkoutSessionId, amount, currency, returnUrl, cancelUrl) {
...
export async function getPayment(paymentId) {
	const response = await fetchWithCredentials(`${API_BASE}/api/v1/payments/${paymentId}`, {
		method: 'GET'
	});
...
export async function capturePayment(paymentId, orderId) {
	const response = await fetchWithCredentials(`${API_BASE}/api/v1/payments/${paymentId}/capture`, {
		method: 'POST',
